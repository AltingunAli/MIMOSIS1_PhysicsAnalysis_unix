...
public:
	...
	HistoPlotter(Matrix *MIMOSIS1, std::vector<MIMOSIS1_Single_Run_Dataset> &v_MIM_int_frame, std::vector<int> &&v_param_values) : _MIMOSIS1(MIMOSIS1), _v_MIM_int_frame(v_MIM_int_frame), _v_param_values(v_param_values){};
	...
	TH1D *h_noise_sigma_clean;	//!< 1-D histo of pixel noise (from selected part of matrix), Altingun, 2024
	TH1D *h_mu;					//!< 1-D histo of pixel mean (from selected part of matrix)
	TH1D *h_mu_clean;			//!< 1-D histo of pixel mean (from selected part of matrix), Altingun, 2024
	...
	TMultiGraph *mg_scurves_clean; //!< Multigraph of all S-curves with fits, Altingun, 2024
...
private:
	...
	double _dac_shift;
	double _VPH_Fine_mV;// Altingun, 2024
